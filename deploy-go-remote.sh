...
GOOS=linux GOARCH=amd64 go build -o picshare-server ./main.go
...
tar -czf /tmp/picshare-deploy.tar.gz \
  ...
  --exclude='*.md' \
  --exclude='backend' \
  --exclude='backend-go/vendor' \
  frontend/dist/ \
  backend-go/picshare-server \
  deploy.sh
...
# Step 5: Run deployment on server
echo ""
echo "🚀 执行部署..."
...
echo "   3. 查看日志: ssh ${ECS_USER}@${ECS_HOST} 'journalctl -u picshare -f'"
