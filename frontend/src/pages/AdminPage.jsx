...
import {
  ...
  ChevronRight, BarChart3, Activity, Camera
} from 'lucide-react';
...
export default function AdminPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [statsRes, usersRes, albumsRes] = await Promise.all([
        adminAPI.getStats(),
        adminAPI.getUsers(),
        adminAPI.getAllAlbums(),
      ]);
                ...
                {users.map((user) => (
                  <>
                    <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <button
                          onClick={() => fetchUserAlbums(user.id)}
                    ...
                    {selectedUser === user.id && (
                      <tr key={`${user.id}-albums`}>
                        <td colSpan="7" className="bg-gray-50 px-4 py-3">
                          {userAlbums.length === 0 ? (
                            <p className="text-sm text-gray-500 text-center py-2">暂无影集</p>
                          ) : (
                                ...
                                <div key={a.id} className="flex items-center justify-between bg-white rounded-lg px-3 py-2 text-sm">
                                  <div>
                                    ...
                                    <span className={`ml-2 text-xs px-2 py-0.5 rounded-full ${
                                      a.isExpired ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600'
                                    }`}>
                                      {a.isExpired ? '已过期' : '活跃'}
                                    </span>
                                  </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase hidden md:table-cell">过期时间</th>
                ...
                {albums.map((album) => (
                  <tr key={album.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-600 hidden sm:table-cell">
                      <div>{album.photographer_name}</div>
                      <div className="text-xs text-gray-400">{album.photographer_email}</div>
                    </td>
                    ...
                    <td className="px-4 py-3 text-xs text-gray-500 hidden md:table-cell">
                      {new Date(album.expires_at).toLocaleString('zh-CN')}
                    </td>
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-1 rounded-full font-medium ${
                        album.isExpired ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600'
                      }`}>
                        {album.isExpired ? '已过期' : '活跃'}
                      </span>
                    </td>
                  </tr>
                ))}
          ...
          {albums.length === 0 && (
            <div className="text-center py-10">
              ...
              <p className="text-gray-500">暂无影集</p>
            </div>
          )}
        </div>
      )}
